...
The first remote Linux mutator uses Ranch Hand's native SSH client; Windows does not need OpenSSH, WSL, Docker, Compose, or a Linux shell. The target Linux account must already be able to run Docker and Docker Compose v2. Ranch Hand can inspect the host key presented by the SSH service without sending credentials. The operator verifies that fingerprint against a trusted source such as the Azure VM console, physical/virtual server console, or server administrator, then pins it in the secret-free plan. The SSH key or password entered later authenticates the user account; it does not replace server-identity verification. Live preflight verifies the pinned identity, Linux Docker Engine and Compose, unused project, and dedicated installation directory.
