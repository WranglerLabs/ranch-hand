...
const targetFields: Record<string, { key: string; label: string; placeholder: string; optional?: boolean }[]> = {
  "azure-container-apps": [
    ...
    { key: "postgresServerName", label: "PostgreSQL server name (production data mode)", placeholder: "repo-wrangler-prod-unique", optional: true },
  ],
  cloudflare: [
...
const targetDefaults: Record<string, Record<string, string>> = {
  "azure-container-apps": { demoMode: "false" },
  cloudflare: { demoMode: "false" },
  "local-compose": { projectName: "repo-wrangler", dataVolume: "repo-wrangler-data", listenAddress: "127.0.0.1:8080", demoMode: "false" },
  "local-wsl-compose": { projectName: "repo-wrangler-ranch-hand", demoMode: "false" },
  "remote-linux-compose": { port: "22", projectName: "repo-wrangler-ranch-hand", accessMode: "private-lan", demoMode: "false" },
};
...
function wslApplicationURL(demoMode?: string) {
  return demoMode === "true" ? "http://127.0.0.1:8080" : "http://127.0.0.1:8080/onboarding";
}
...
function App() {
  ...
  const [localInstallMessage, setLocalInstallMessage] = useState("");
  ...
  const [removalCredentials, setRemovalCredentials] = useState<Record<string, Record<string, string>>>({});
  ...
  function removalCredentialsReady(record: InstallationRecord) {
    const credentials = removalCredentials[record.deploymentId] || {};
    if (record.target === "azure-container-apps") return Boolean(credentials.azureAccessToken?.trim());
    if (record.target === "cloudflare") return Boolean(credentials.cloudflareApiToken?.trim());
    if (record.target === "remote-linux-compose") return Boolean(credentials.sshPassword?.trim() || credentials.sshPrivateKey?.trim());
    return true;
  }

  function setRemovalCredential(deploymentId: string, key: string, value: string) {
    setRemovalCredentials((current) => ({ ...current, [deploymentId]: { ...(current[deploymentId] || {}), [key]: value } }));
  }

  async function uninstallDeployment(record: InstallationRecord) {
    setRemovingDeployment(record.deploymentId);
    setRemovalMessage(`Removing the owned ${record.target} deployment and its persistent data. Keep Ranch Hand open.`);
    try {
      await api<{ completed: boolean }>(`/api/v1/installations/${record.deploymentId}/uninstall`, {
        method: "POST",
        body: JSON.stringify({ confirmed: true, deleteData: true, credentials: removalCredentials[record.deploymentId] || {} }),
      });
      setRemovalConfirmations((current) => ({ ...current, [record.deploymentId]: false }));
      setRemovalCredentials((current) => { const next = { ...current }; delete next[record.deploymentId]; return next; });
      setRemovalMessage("The deployment and its persistent data were removed. Ranch Hand closed the active installation record.");
      await refreshInstallations();
      await refreshActiveOperations();
    } catch (reason) {
      setRemovalMessage(reason instanceof Error ? reason.message : "Managed removal failed");
    } finally {
  ...
  async function installLocal() {
    if (!planResult) {
      setLocalInstallMessage("Create and preflight the Docker Desktop deployment plan before installing.");
      return;
    }
    if (!installConfirmed) {
      setLocalInstallMessage("Select the confirmation checkbox before starting the Docker Desktop installation.");
      return;
    }
    setInstalling(true);
    setLocalInstallMessage("Creating the Docker container and waiting for RepoWrangler readiness. Keep this Ranch Hand window open.");
    setPlanError("");
    setOperationResult(null);
    try {
      ...
      setLocalInstallMessage("");
    } catch (reason) {
      const message = reason instanceof Error ? reason.message : "Local installation failed";
      setPlanError(message);
      setLocalInstallMessage(message);
    } finally {
      setInstalling(false);
    }
  }
  ...
  async function installAzure() {
    ...
    try {
      setOperationResult(await api<OperationResult>("/api/v1/operations/run", {
        method: "POST",
        body: JSON.stringify({ kind: "install", plan: planResult, credentials: { azureAccessToken: operationAzureToken, setupToken } }),
      }));
      setOperationKind("azure-install");
      setOperationAzureToken("");
    } catch (reason) {
      setPlanError(reason instanceof Error ? reason.message : "Azure installation failed");
    } finally {
      setInstalling(false);
    }
  }
  ...
  async function installCloudflare() {
    ...
    try {
      setOperationResult(await api<OperationResult>("/api/v1/operations/run", {
        method: "POST",
        body: JSON.stringify({ kind: "install", plan: planResult, credentials: { cloudflareApiToken: operationCloudflareToken, setupToken } }),
      }));
      setOperationKind("cloudflare-install");
      setOperationCloudflareToken("");
    } catch (reason) {
      setPlanError(reason instanceof Error ? reason.message : "Cloudflare installation failed");
    } finally {
      setInstalling(false);
    }
  }
  ...
  return (
    <main>
      ...
      {installations.some((record) => record.state === "active") && <section className="release-panel" aria-labelledby="deployments-heading"><p className="eyebrow">Lifecycle inventory</p><h2 id="deployments-heading">Managed deployments</h2><p>These durable records are Ranch Hand's authoritative deployment inventory. Every listed target can be removed here with an ownership check and explicit permanent-data confirmation.</p>{installations.filter((record) => record.state === "active").map((record) => { const values = removalCredentials[record.deploymentId] || {}; return <div className="inline-result success" key={record.deploymentId}><strong>{record.plan.name}</strong><dl><div><dt>Target</dt><dd>{record.target}</dd></div><div><dt>Project</dt><dd>{record.plan.configuration.projectName || record.plan.configuration.appName || record.plan.configuration.workerName || "Managed target"}</dd></div><div><dt>Version</dt><dd>{record.version}</dd></div><div><dt>State</dt><dd>{record.state}</dd></div><div><dt>Updated</dt><dd>{new Date(record.updatedAt).toLocaleString()}</dd></div></dl>{record.target === "local-wsl-compose" && <a href={wslApplicationURL(record.plan.configuration.demoMode)} target="_blank" rel="noreferrer">Open RepoWrangler</a>}{record.target === "local-compose" && <a href={`http://${record.plan.configuration.listenAddress}`} target="_blank" rel="noreferrer">Open RepoWrangler</a>}{record.target === "remote-linux-compose" && record.plan.configuration.accessMode === "private-lan" && <a href={`http://${record.plan.configuration.host}:8080`} target="_blank" rel="noreferrer">Open RepoWrangler on {record.plan.configuration.host}</a>}<div className="install-panel"><strong>Remove this deployment</strong><p>This permanently deletes only resources whose Ranch Hand ownership identity matches this exact deployment, including its persistent data.</p>{record.target === "azure-container-apps" && <label>Temporary Azure ARM access token<input type="password" value={values.azureAccessToken || ""} onChange={(event) => setRemovalCredential(record.deploymentId, "azureAccessToken", event.target.value)} /></label>}{record.target === "cloudflare" && <label>Scoped Cloudflare API token<input type="password" value={values.cloudflareApiToken || ""} onChange={(event) => setRemovalCredential(record.deploymentId, "cloudflareApiToken", event.target.value)} /></label>}{record.target === "remote-linux-compose" && <><label>SSH password<input type="password" value={values.sshPassword || ""} onChange={(event) => setRemovalCredential(record.deploymentId, "sshPassword", event.target.value)} /></label><label>SSH private key (alternative)<textarea value={values.sshPrivateKey || ""} onChange={(event) => setRemovalCredential(record.deploymentId, "sshPrivateKey", event.target.value)} /></label><label>Private-key passphrase (if encrypted)<input type="password" value={values.sshPrivateKeyPassphrase || ""} onChange={(event) => setRemovalCredential(record.deploymentId, "sshPrivateKeyPassphrase", event.target.value)} /></label></>}<label className="confirmation"><input type="checkbox" checked={Boolean(removalConfirmations[record.deploymentId])} onChange={(event) => setRemovalConfirmations((current) => ({ ...current, [record.deploymentId]: event.target.checked }))} /> I understand this permanently deletes this deployment and its RepoWrangler data.</label><button type="button" className="danger" disabled={!removalConfirmations[record.deploymentId] || !removalCredentialsReady(record) || removingDeployment !== ""} onClick={() => uninstallDeployment(record)}>{removingDeployment === record.deploymentId ? "Removing deployment…" : "Permanently remove deployment"}</button></div></div>; })}</section>}
      ...
      {artifact && <section className="release-panel" aria-labelledby="plan-heading">
        ...
        <form className="plan-form" onSubmit={createPlan}>
          ...
          <label className="confirmation"><input type="checkbox" checked={configuration.demoMode === "true"} onChange={(event) => changeConfiguration({ ...configuration, demoMode: event.target.checked ? "true" : "false" })} /> Demo mode — use mock data instead of connecting real repositories.</label>
          <p>{configuration.demoMode === "true" ? "Demo mode is an optional evaluation profile and uses mock data without provider setup." : "Production data mode is selected. Ranch Hand will provision the target's persistent data contract and required secrets, then RepoWrangler will guide you through first-run provider setup."}</p>
        ...
        {target === "local-compose" && targetReport?.ready && stagedBundle && !operationResult && <div className="inline-result install-panel"><strong>Apply Docker Desktop plan</strong><label>Operation<select value={localAction} onChange={(event) => { setLocalAction(event.target.value as "install" | "update"); setInstallConfirmed(false); setLocalInstallMessage(""); }}><option value="install">New installation</option><option value="update">Backup-first update</option></select></label>{localAction === "install" ? <><p>This uses Docker Desktop's Windows-exposed Docker Engine API, installs RepoWrangler with SQLite in <strong>{planResult?.configuration.demoMode === "true" ? "demo mode" : "real setup mode"}</strong>, and binds only to 127.0.0.1.</p><label className="confirmation"><input type="checkbox" checked={installConfirmed} onChange={(event) => { setInstallConfirmed(event.target.checked); setLocalInstallMessage(""); }} /> Install this local {planResult?.configuration.demoMode === "true" ? "demo" : "real-data"} deployment with Docker Desktop.</label><button type="button" disabled={!installConfirmed || installing || inventoryLoading || currentInstallation !== null} onClick={installLocal}>{installing ? "Installing and verifying…" : inventoryLoading ? "Checking installation inventory…" : "Install with Docker Desktop"}</button>{localInstallMessage && <p className={installing ? "operation-progress" : "operation-warning"} role={installing ? "status" : "alert"} aria-live="polite">{localInstallMessage}</p>}</> : <><p>Ranch Hand will verify and back up the current owned container, seed a new volume, preserve the old container and volume for rollback, activate the immutable release selected above, and recover automatically if readiness fails.</p><label>Recorded currently installed immutable version<input readOnly value={fromVersion} placeholder={inventoryLoading ? "Loading installation record…" : "No active installation record"} /></label><label className="confirmation"><input type="checkbox" checked={installConfirmed} onChange={(event) => setInstallConfirmed(event.target.checked)} /> I understand the running local instance will have brief downtime during backup and activation.</label><button type="button" disabled={!installConfirmed || !fromVersion || fromVersion === planResult?.release.version || installing} onClick={updateLocal}>{installing ? "Backing up and updating…" : "Back up and update local deployment"}</button></>}</div>}
        ...
        {target === "azure-container-apps" && targetReport?.ready && stagedBundle && !operationResult && <div className="inline-result install-panel"><strong>Azure target is ready to install</strong><p>The subscription, resource provider, names, and Azure-managed HTTPS contract passed preflight. Ranch Hand retained the successful ARM token only in this running local session. {planResult?.configuration.demoMode === "true" ? "Demo mode will use SQLite and mock data." : "Production data mode will provision a dedicated PostgreSQL flexible server, protected Container App secrets, and real provider onboarding."} Azure resources are billable.</p><label className="confirmation"><input type="checkbox" checked={installConfirmed} onChange={(event) => setInstallConfirmed(event.target.checked)} /> Create and install this dedicated Azure {planResult?.configuration.demoMode === "true" ? "demo" : "production-data"} environment.</label><button type="button" disabled={!installConfirmed || !operationAzureToken || installing} onClick={installAzure}>{installing ? "Installing and verifying…" : "Install on Azure"}</button></div>}
        {target === "cloudflare" && targetReport?.ready && stagedBundle && !operationResult && <div className="inline-result install-panel"><strong>Cloudflare target is ready to install</strong><p>The account, workers.dev endpoint, Worker name, and D1 name passed preflight. Ranch Hand retained the successful scoped token only in this running local session. It will create the dedicated Worker and D1 database in <strong>{planResult?.configuration.demoMode === "true" ? "demo mode" : "production data mode"}</strong>, publish the verified bundle, and install protected runtime secrets for production data mode.</p><label className="confirmation"><input type="checkbox" checked={installConfirmed} onChange={(event) => setInstallConfirmed(event.target.checked)} /> Create and install this dedicated Cloudflare {planResult?.configuration.demoMode === "true" ? "demo" : "production-data"} environment.</label><button type="button" disabled={!installConfirmed || !operationCloudflareToken || installing} onClick={installCloudflare}>{installing ? "Installing and verifying…" : "Install on Cloudflare"}</button></div>}
        {target === "remote-linux-compose" && targetReport?.ready && stagedBundle && !operationResult && <div className="inline-result install-panel"><strong>Ready to install on {planResult?.configuration.user}@{planResult?.configuration.host}</strong><p>The SSH connection, Docker Engine, Compose, directory, project, and private-network boundary checks passed. RepoWrangler will be reachable from this private network at <code>http://{planResult?.configuration.host}:8080</code>. Ranch Hand retained the successful credential only in this running local session so you do not need to enter it again.</p><label className="confirmation"><input type="checkbox" checked={installConfirmed} onChange={(event) => { setInstallConfirmed(event.target.checked); setRemoteInstallMessage(""); }} /> Install RepoWrangler and expose port 8080 on this private server address.</label><button type="button" disabled={!installConfirmed || installing} onClick={installRemote}>{installing ? "Installing and verifying…" : "Install RepoWrangler"}</button>{remoteInstallMessage && <p className={installing ? "operation-progress" : "operation-warning"} role={installing ? "status" : "alert"} aria-live="polite">{remoteInstallMessage}</p>}{installing && <p className="operation-progress" role="status" aria-live="polite">{(() => { const operation = activeOperations.find((item) => item.deploymentId === targetReport.deploymentId); return operation ? `Current lifecycle phase: ${operation.phase}. Ranch Hand is still working; keep this window open.` : "Creating the durable operation journal…"; })()}</p>}</div>}
        {operationResult && operationKind === "install" && <div className="inline-result success"><strong>Docker Desktop RepoWrangler installation committed</strong><p>The container passed its readiness check and the lifecycle journal is {operationResult.operation.journal.phase}. Open <a href={planResult?.configuration.demoMode === "true" ? `http://${planResult?.configuration.listenAddress}` : `http://${planResult?.configuration.listenAddress}/onboarding`} target="_blank" rel="noreferrer">{planResult?.configuration.demoMode === "true" ? `http://${planResult?.configuration.listenAddress}` : `http://${planResult?.configuration.listenAddress}/onboarding`}</a>.</p><button type="button" className="secondary" disabled={installing} onClick={backupLocal}>{installing ? "Creating consistent backup…" : "Back up local data"}</button></div>}
        {operationResult && operationKind === "wsl-install" && <div className="inline-result success"><strong>WSL Docker Compose installation committed</strong><p>Docker Compose started the verified RepoWrangler release inside {planResult?.configuration.distribution}. Open <a href={wslApplicationURL(planResult?.configuration.demoMode)} target="_blank" rel="noreferrer">{wslApplicationURL(planResult?.configuration.demoMode)}</a>.</p></div>}
        ...
        {operationResult && operationKind === "azure-install" && <div className="inline-result success"><strong>Azure installation committed</strong><p>The ARM deployment, digest-pinned image, Azure-managed HTTPS endpoint, readiness, and exact immutable release identity passed verification. The lifecycle journal is {operationResult.operation.journal.phase}.</p>{planResult?.configuration.demoMode === "false" && remoteSetupToken && <p><strong>Initial setup token:</strong> <code>{remoteSetupToken}</code><br /><small>Enter this token during RepoWrangler onboarding. It remains only in this Ranch Hand window and the Azure Container App secret store.</small></p>}</div>}
        {operationResult && operationKind === "cloudflare-install" && <div className="inline-result success"><strong>Cloudflare installation committed</strong><p>The D1 ownership marker and migrations, Worker module and assets, schedules, Cloudflare-managed HTTPS endpoint, readiness, and exact immutable release identity passed verification. The lifecycle journal is {operationResult.operation.journal.phase}.</p>{planResult?.configuration.demoMode === "false" && remoteSetupToken && <p><strong>Initial setup token:</strong> <code>{remoteSetupToken}</code><br /><small>Enter this token during RepoWrangler onboarding. It remains only in this Ranch Hand window and the Cloudflare Worker secret binding.</small></p>}</div>}
