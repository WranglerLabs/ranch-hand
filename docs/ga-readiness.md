...
Ranch Hand is a **Public Preview**. It is the primary recommended
...
| RepoWrangler compatibility | Ranch Hand installs and manages the latest supported RepoWrangler patch through its immutable manifest and rejects unsupported, floating, downgraded, or tampered inputs | Partial; immutable release discovery, verification, and target identity checks pass, while exact-release clean-target UAT remains required |
